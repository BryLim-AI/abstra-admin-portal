...
export default async function tenantBillingAnalytics(req, res) {
    if (req.method !== "GET") {
    ...
    try {
        const [billingHistory] = await db.execute(
            `SELECT DATE_FORMAT(b.billing_period, '%Y-%m') AS month, 
                    SUM(b.total_amount_due) AS total_billed_amount
             FROM Billing b
        ...
        const [billingBreakdown] = await db.execute(
            `SELECT DATE_FORMAT(b.billing_period, '%Y-%m') AS month, 
                    SUM(b.total_water_amount) AS water_amount,
                    SUM(b.total_electricity_amount) AS electricity_amount,
